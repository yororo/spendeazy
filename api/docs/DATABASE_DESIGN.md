...
| `invitations` | Sender-owned Shared Space Invite Codes with seven-day expiry, protected lookup and display representations, and one pending invitation per sender. Pending invitations do not reserve membership. |
| `invitation_claims` | Per-User saved incoming invitation entries. A unique invitation/User pair makes repeated code entry idempotent while allowing multiple Users to save the same active code. |
