...
The current implementation scopes financial ownership and duplicate checks to Users. Space ownership will require coordinated browser/API changes. This accepted target design supersedes the per-User serialization scope and last-write-wins replacement in [the API Category Rule ADR](../../api/docs/adr/0003-category-rule-storage-and-replacement.md): rule mutations will be scoped to the Space and detect stale edits. It also extends the ownership boundary in [the web authentication ADR](../../web/docs/adr/0002-authenticated-self-scoped-api-boundary.md) to authorized Spaces while preserving independent session identities and centralized authentication. Current code has not yet adopted these changes.
